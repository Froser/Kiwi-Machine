...
FileVersionInfo::CreateFileVersionInfoForModule(HMODULE module) {
  ...
  const bool has_version_resource = kiwi::base::win::GetResourceFromModule(
...
FileVersionInfoWin::CreateFileVersionInfoWin(
  ...
  return kiwi::base::WrapUnique(new FileVersionInfoWin(
...
bool FileVersionInfoWin::GetValue(const char16_t* name,
  ...
  for (const auto& lang_codepage : lang_codepages) {
    ...
    _snwprintf_s(sub_block, MAX_PATH, MAX_PATH,
                 ...
                 lang_codepage.code_page, as_wcstr(name));
...
kiwi::base::Version FileVersionInfoWin::GetFileVersion() const {
  return kiwi::base::Version({HIWORD(fixed_file_info_.dwFileVersionMS),
