...
#define DPLOG_IF(S, R) \
  if (R)               \
  LOG(S)
#define DPLOG(S) DLOG(S)

namespace {

struct timespec ToTimeSpec(const kiwi::base::TimeDelta& td) {
  int64_t microseconds = td.InMicroseconds();
  time_t seconds = 0;
  if (microseconds >= kiwi::base::TimeBase::kMicrosecondsPerSecond) {
    seconds = static_cast<time_t>(td.InSeconds());
    microseconds -= seconds * kiwi::base::TimeBase::kMicrosecondsPerSecond;
  }
  struct timespec result = {
      seconds,
      static_cast<long>(microseconds *
                        kiwi::base::TimeBase::kNanosecondsPerMicrosecond)};
  return result;
}

bool WaitpidWithTimeout(kiwi::base::ProcessHandle handle,
                        int* status,
                        kiwi::base::TimeDelta wait) {
  ...
  if (wait == kiwi::base::TimeDelta::Max()) {
  ...
  std::chrono::time_point wakeup_time = std::chrono::system_clock::now();
  ...
  while (ret_pid == 0) {
    ...
    const uint32_t sleep_time_usecs = static_cast<uint32_t>(
        std::min(static_cast<uint64_t>((wakeup_time - now).count()),
...
bool WaitForSingleNonChildProcess(kiwi::base::ProcessHandle handle,
                                  kiwi::base::TimeDelta wait) {
  DCHECK_GT(handle, 0);

  kiwi::base::ScopedFD kq(kqueue());
  if (!kq.is_valid()) {
    DPLOG(ERROR) << "kqueue";
    return false;
  }
  ...
  // interrupted.
  bool wait_forever = (wait == kiwi::base::TimeDelta::Max());
  kiwi::base::TimeDelta remaining_delta;
  // base::TimeTicks deadline;
  std::chrono::time_point<std::chrono::system_clock> deadline;
  if (!wait_forever) {
    remaining_delta = wait;
    deadline = std::chrono::system_clock::now() +
               std::chrono::microseconds(remaining_delta.InMicroseconds());
  }
  ...
  do {
    ...
    if (wait_forever) {
      ...
      remaining_timespec = ToTimeSpec(remaining_delta);
    ...
    if (result == -1 && errno == EINTR) {
      if (!wait_forever) {
        ...
        remaining_delta = kiwi::base::Microseconds(
            (deadline - std::chrono::system_clock::now()).count());
      }
      result = 0;
    } else {
      break;
...
bool WaitForExitWithTimeoutImpl(kiwi::base::ProcessHandle handle,
                                int* exit_code,
                                kiwi::base::TimeDelta timeout) {
  const kiwi::base::ProcessHandle our_pid =
      kiwi::base::GetCurrentProcessHandle();
  if (handle == our_pid) {
  ...
  const kiwi::base::ProcessHandle parent_pid =
      kiwi::base::GetParentProcessId(handle);
