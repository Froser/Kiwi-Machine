...
class BufferIterator {
 public:
  ...
  BufferIterator(B* data, size_t size)
      : BufferIterator(std::span(data, size)) {}
  explicit BufferIterator(std::span<B> buffer)
  ...
  template <typename T,
            typename = std::enable_if_t<std::is_trivially_copyable_v<T>>>
  std::span<T> MutableSpan(size_t count) {
    size_t size;
    size = sizeof(T) * count;
    ...
    if (size > remaining_.size())
      return std::span<T>();
    auto result = std::span<T>(reinterpret_cast<T*>(remaining_.data()), count);
    remaining_ = remaining_.subspan(size);
    return result;
  }
  ...
  template <typename T,
            typename = std::enable_if_t<std::is_trivially_copyable_v<T>>>
  std::span<const T> Span(size_t count) {
