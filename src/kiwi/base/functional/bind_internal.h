...
BanUnconstructedRefCountedReceiver(const Receiver& receiver, Unused&&...) {
  ...
  // to ref-counted objects, and assert()s otherwise. As above, that typically
  ...
  //
  assert(receiver->HasAtLeastOneRef());
}
 ...
 private:
  ...
  explicit BindState(std::false_type,
      ...
      : BindStateBase(invoke_func, &Destroy),
        functor_(std::forward<ForwardFunctor>(functor)),
        bound_args_(std::forward<ForwardBoundArgs>(bound_args)...) {
    // See above for assert/assert rationale.
    if (is_nested_callback) {
      assert(!IsNull(functor_));
    } else {
      assert(!IsNull(functor_));
...
// bound arguments. We assert() the validity of callback to guard against null
