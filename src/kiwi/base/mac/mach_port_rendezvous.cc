...
#if !defined(MACH_DCHECK)
#define MACH_DCHECK(cond, kr) DCHECK(cond) << kr
#define BOOTSTRAP_CHECK(cond, kr) DCHECK(cond) << kr
#define MACH_LOG(s, mr) LOG(s) << mr
#define BOOTSTRAP_LOG(s, kr) LOG(s) << kr
#endif

namespace kiwi::base {

namespace {
// TODO Change Bundle id here
const char* BaseBundleID() {
  return "org.bytedance.Doctor";
}
...
MachPortRendezvousServer::MachPortRendezvousServer() {
  std::string bootstrap_name =
      StringPrintf(kBootstrapNameFormat, BaseBundleID(), getpid());
...
std::string MachPortRendezvousClient::GetBootstrapName() {
  return StringPrintf(kBootstrapNameFormat, BaseBundleID(), getppid());
}
