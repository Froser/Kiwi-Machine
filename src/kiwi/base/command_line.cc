...
void CommandLine::set_slash_is_not_a_switch() {
  ...
  static_assert(std::span(kSwitchPrefixes).back() == L"/",
...
void CommandLine::AppendSwitchNative(StringPiece switch_string,
  ...
  if (!value.empty()) {
    // Replaces by following
    // base::StrAppend(&combined_switch_string, {kSwitchValueSeparator, value});
    combined_switch_string = kSwitchValueSeparator + StringType(value);
  }
  // Append the switch and update the switches/arguments divider |begin_args_|.
  argv_.insert(argv_.begin() + begin_args_, combined_switch_string);

  // We haven't implemented CheckedNumeric yet
  // begin_args_ = (CheckedNumeric(begin_args_) + 1).ValueOrDie();
  begin_args_ = ((begin_args_) + 1);
}
...
void CommandLine::CopySwitchesFrom(const CommandLine& source,
                                   std::span<const char* const> switches) {
  ...
  for (size_t i = 1; i < argv_.size(); ++i) {
    ...
    if (parse_switches && IsSwitch(arg, &switch_string, &switch_value)) {
      ...
      if (has_single_argument_switch_) {
        ...
        params += StringType(kSwitchPrefixes[0]) + StringType(kSingleArgument) +
                  FILE_PATH_LITERAL(" ");
