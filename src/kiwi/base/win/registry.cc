...
RegKey::RegKey(RegKey&& other) noexcept
    : key_(other.key_), wow64access_(other.wow64access_) {
  other.key_ = nullptr;
...
std::variant<bool, LONG> RegKey::IsLink() const {
  DWORD value_type = 0;
  LONG result = ::RegQueryValueEx(key_, L"SymbolicLinkValue",
                                  /*lpReserved=*/nullptr, &value_type,
                                  /*lpData=*/nullptr, /*lpcbData=*/nullptr);
  if (result == ERROR_FILE_NOT_FOUND) {
    return std::variant<bool, LONG>(false);
  }
  if (result == ERROR_SUCCESS) {
    return std::variant<bool, LONG>(value_type == REG_LINK);
  }
  return std::variant<bool, LONG>(result);
}

std::optional<LONG> RegKey::DeleteIfLink() {
  auto is_link = IsLink();
  bool* link_has_value = std::get_if<bool>(&is_link);
  if (auto is_link = IsLink(); !link_has_value) {
    return std::get<LONG>(is_link);  // Failed to determine if a link.
  } else if (std::get<bool>(is_link) == false) {
