...
void Nes_Apu::run_until( cpu_time_t end_time )
{
	...
	while ( true )
	{
		...
		// run oscs to present
		if (audio_channels_mask & audio_channel_square_1)
			square1.run( last_time, time );
		if (audio_channels_mask & audio_channel_square_2)
			square2.run( last_time, time );
		if (audio_channels_mask & audio_channel_triangle)
			triangle.run( last_time, time );
		if (audio_channels_mask & audio_channel_noise)
			noise.run( last_time, time );
		if (audio_channels_mask & audio_channel_dmc)
...
void Nes_Apu::set_audio_channels(int channels) {
	audio_channels_mask = channels;
}

int Nes_Apu::audio_channels() {
	return audio_channels_mask;
}
