...
TEST(PaletteTest, ReturnsCanonical2C02Colors) {
  constexpr std::array<Color, 64> kExpectedColors = {
  ...
  std::unique_ptr<Palette> palette = CreatePaletteFromPPUModel(PPUModel::k2C02);

  for (size_t index = 0; index < kExpectedColors.size(); ++index) {
    EXPECT_EQ(palette->GetColorBGRA(index), kExpectedColors[index])
        << "palette index " << index;
  }
}
