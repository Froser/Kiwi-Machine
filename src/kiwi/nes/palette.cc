...
namespace nes {
namespace {
constexpr Color k2C02ColorsBGRA[] = {
...
static_assert(sizeof(k2C02ColorsBGRA) / sizeof(Color) == Palette::kColorCount);
}  // namespace

Palette::Palette(PPUModel model) : model_(model) {
  switch (model_) {
    case PPUModel::k2C02:
      colors_ = k2C02ColorsBGRA;
      break;
  }
  CHECK(colors_) << "Unsupported PPU model.";
}
Palette::~Palette() = default;

Color Palette::GetColorBGRA(int index) const {
  DCHECK(index >= 0 && index < kColorCount);
  return colors_[index];
}

std::unique_ptr<Palette> CreatePaletteFromPPUModel(PPUModel ppu) {
  return std::make_unique<Palette>(ppu);
}
}  // namespace nes
}  // namespace kiwi
